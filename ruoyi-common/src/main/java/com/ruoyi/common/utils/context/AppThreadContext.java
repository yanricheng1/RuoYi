...
public class AppThreadContext {
    public static final String ACCOUNT_KEY = AppThreadContext.class.getName() + "_$_ACCOUNT_KEY_$";
    public static final String TENANT_KEY = AppThreadContext.class.getName() + "_$_TENANT_KEY_$";
    private static final Logger log = LoggerFactory.getLogger(AppThreadContext.class);
    private static final ThreadLocal<Map<Object, Object>> resources = new InheritableThreadLocalMap<Map<Object, Object>>();

    /**
     * Default no-argument constructor.
     */
    protected AppThreadContext() {
    }
     ...
     * }</pre>
     *
     * @param Account the application's SecurityManager instance to bind to the thread.  If the argument is
     ...
     * <p/>
     * This method only returns the bound value if it exists - it does not remove it
     * from the thread.  To remove it, one must call {@link #unbindTenant() unbindSubject()} instead.
     *
     * @return the Subject object bound to the thread, or <tt>null</tt> if there isn't one bound.
     * @since 0.2
     */
    public static Tenant getTenant() {
        return (Tenant) get(TENANT_KEY);
    }
     ...
     * }</pre>
     *
     * @param tenant the Subject object to bind to the thread.  If the argument is null, nothing will be done.
     * @since 0.2
     */
    public static void bind(Tenant tenant) {
        if (tenant != null) {
     ...
     * thread execution), you should use the {@link #getTenant() getSubject()} method for that purpose.
     *
     * @return the Subject object previously bound to the thread, or <tt>null</tt> if there was none bound.
     * @since 0.2
     */
    public static Tenant unbindTenant() {
        return (Tenant) remove(TENANT_KEY);
    }
